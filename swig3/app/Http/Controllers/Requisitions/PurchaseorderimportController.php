    ...
    public function index(Request $request) {
        try {
            ...
            $purchaseorders = DB::table('ac_purchase_order as ord')
                    ...
                    ->where('requisition.order_type','=',3)
            ...
            if ($request->ajax()) {
                ...
                return view('requisitions/purchase_order/import/results', array('purchaseorders' => $purchaseorders));
            }
            return view('requisitions/purchase_order/import/index', array('purchaseorders' => $purchaseorders));
        } catch (\Exception $e) {
...
//                        'other_ways' => $arraData['other_ways'],
    ...
    public function exportdata_purchaseorder() {
        ...
        if($excelorpdf=="Excel"){
            ...
            Excel::create('Import Purchase Order', function($excel) use($purchaseorders){
                 // Set the title
                $excel->setTitle('Import Purchase Order');
                
                $excel->sheet('Import Purchase Order', function($sheet) use($purchaseorders){
                    // Sheet manipulation
                    
                    $sheet->setCellValue('C3', 'Import Purchase Order');
