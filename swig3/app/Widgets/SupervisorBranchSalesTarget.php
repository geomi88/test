...
class SupervisorBranchSalesTarget extends AbstractWidget
{
    /**
    ...
    public function run()
    {
        ...
        if ($employee_data-> privilege_status == 1) {
            ...
            $branches = DB::table('resource_allocation')
                    ->select('resource_allocation.branch_id', 'master_resources.name as branch_name')
                    ->leftjoin('master_resources', 'master_resources.id', '=', 'resource_allocation.branch_id')
                    ->whereRaw("resource_allocation.employee_id = $employee_id and resource_allocation.active = 1")
                    ->orderby('master_resources.name')
                    ->groupBy('resource_allocation.branch_id')
                    ->get();
        }
        $full_result = array();
        ...
        return view('widgets.supervisor_branch_sales_target', [
