...
class LowSales extends AbstractWidget {
    ...
    public function run() {
        ...
        foreach ($low_sales as $low_sale) {
            $branches = $branches . "'" . $low_sale->branch_name . "',";
            $total_sale = $total_sale . $low_sale->total . ",";
        }
        $branches = trim($branches, ',');
        $total_sale = trim($total_sale, ',');


        return view("widgets.low_sales", [
