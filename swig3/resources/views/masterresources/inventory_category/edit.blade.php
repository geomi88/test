...
<div class="innerContent">
    ...
    <form action="{{ action('Masterresources\InventorycategoryController@update') }}" method="post" id="inventorycatupdation">
            ...
            <div class="custRow">
                <div class="custCol-4">
                    <div class="inputHolder inventorycatName">
                        ...
                        <input type="text" name="name" id="name" value='{{ $categorie_datas->name }}' onpaste="return false;" autocomplete="off" placeholder="Enter Name">
                ...
                <div class="custCol-4">
                    <div class="inputHolder">
                        <label>Alias</label>
                        <input type="text" name="alias_name" value='{{ $categorie_datas->alias_name }}' id="alias_name" placeholder="Enter Alias Name">
                    </div>
                </div>
                <div class="custCol-4">
                    <div class="inputHolder bgSelect">
                        <label>Inventory Category</label>
                        <input type="text" id="txtcategoryname" value="{{$categorie_datas->category_name}}" name="txtcategoryname" placeholder="Select Inventory Category" readonly="true">
                        <input type="hidden" id="inventory_category_id" name="inventory_category_id" value="{{$categorie_datas->inventory_category_id}}">
                        <span class="commonError"></span>
                    </div>
                </div>
            </div>
...
<script>
    ...
    $('#inventorycatupdation').on('submit', function (event) {
        if ($("#cid").val() == $("#inventory_category_id").val()) {
            if (!$("#inventorycatupdation").valid()) {
                return;
            }
            event.preventDefault();
            alert("Category and main category cannot be same");
