...
<script>
    ...
    $(document).ready(function ()
    {
     ...
     $('.takePrint').click(function () { 
        ...
        if(!$("#slNo").is(':checked')&&!$("#jobPosition").is(':checked')&&!$("#jobCode").is(':checked')&&!$("#alias").is(':checked')) {
        ...
        var strStyle='<style>.paginationHolder {display:none;}';
         ...
         if(!$("#slNo").is(':checked')){
                strStyle+=' .job_slno {display:none;}';
         }
         if(!$("#jobPosition").is(':checked')){
                strStyle+=' .job_name {display:none;}';
         }
         if(!$("#jobCode").is(':checked')){
               strStyle+=' .job_code {display:none;}';
         }
         if(!$("#alias").is(':checked')){
               strStyle+=' .job_alias {display:none;}';
         }
         strStyle+='.actionBtnSet{display:none;} .listerType1 tr:nth-of-type(2n) {background: rgba(0, 75, 111, 0.12) none repeat 0 0;}</style>';
         
    win.document.write(strStyle+'<div style="text-align:center;"><h1>Job Positions</h1></div><table style="width: 100%; border: 3px solid #3088da" cellspacing="0" cellpadding="0" class="listerType1">'+
                                    '<thead style="background: rgba(0, 0, 0, 0) linear-gradient(to bottom, rgba(109, 179, 242, 1) 0%, rgba(84, 163, 238, 1) 50%, rgba(54, 144, 240, 1) 51%, rgba(30, 105, 222, 1) 100%) repeat  0 0;">'+
                                        '<tr class="headingHolder">'+
                                            '<td style="padding:10px 0;color:#fff;" class="job_slno"> Sl.No.</td>'+
                                            '<td style="padding:10px 0;color:#fff;" class="job_name"> Job Position</td>'+
                                            '<td style="padding:10px 0;color:#fff;" class="job_code"> Job Code</td>'+
                                            '<td style="padding:10px 0;color:#fff;" class="job_alias"> Alias </td>'+
                                        '</tr>'+
...
<div class="innerContent">
    <form id="pdfgenerator" action="{{ url('masterresources/job_positions/exportdata') }}" method="post">
    <header class="pageTitle">
        <h1>Job <span>Positions</span></h1>
    </header>
    <div class="fieldGroup" id="fieldSet1">
        <div class="custRow">
            <div class="custCol-12">
                <a href="{{ action('Masterresources\JobpositionController@add') }}" class="right commonBtn commonBtn bgGreen addBtn">Create</a>
            </div>
        </div>
    ...
    <div class="printChoose">
                        <div class="custRow">
                            ...
                            <div class="custCol-4">
                                <div class="commonCheckHolder checkRender">
                                    <label>
                                        <input id="jobCode" checked="" type="checkbox">
                                        <span></span>
                                        <em>Job Code</em>
                                    </label>
                                </div>
            ...
            <div id="tblregion">
                <table cellpadding="0" cellspacing="0" style="width: 100%;">
                    ...
                    <thead class="listHeaderBottom">
                        ...
                        <tr class="headingHolder">
                            ...
                            <td class="filterFields">
                                <div class="custRow">
                                    <div class="custCol-12">
                                        <input type="text" id="searchbyname" name="searchbyname" placeholder="Enter Job Position">
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="filterFields">
                                <div class="custRow">
                                    <div class="custCol-12">
                                        <input type="text" id="searchbycode" name="searchbycode" placeholder="Enter Job Code">
                                    </div>
                                </div>
                            </td>
                    ...
                    <tbody class="jobbody" id='jobbody'>
                        @include('masterresources/job_positions/jobpos_result')
...
<script>
    ...
    function search()
    {
        ...
        $.ajax({
            type: 'POST',
            url: 'job_positions',
            data: {searchbyname: searchbyname, searchbycode:searchbycode,sortordname: sortordname, pagelimit: pagelimit, searchable: searchable},
            beforeSend: function () {
                $(".commonLoaderV1").show();
            },
    ...
    function search1()
    {
        ...
        $.ajax({
            type: 'POST',
            url: 'job_positions',
            data: {searchbyname: searchbyname, searchbycode:searchbycode,sortordercode: sortordercode, pagelimit: pagelimit, searchable: searchable},
            beforeSend: function () {
                $(".commonLoaderV1").show();
            },
            success: function (return_data) {
    ...
    $(function () {
        ...
        $('#reset').click(function () {
            ...
            $('#page-limit').val(10);
            //search();
            window.location.href = '{{url("masterresources/job_positions")}}';
        });
    });
