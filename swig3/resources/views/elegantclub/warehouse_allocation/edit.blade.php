...
<div class="innerContent">
    <header class="pageTitle">
        <h1>Edit Warehouse <span>Allocation</span></h1>
    </header>

    <form action="{{ url('elegantclub/warehose_allocation/store') }}" method="post" id="frmwarehousealloc">
        <input type="hidden" name="ew_id" id="ew_id" value="{{$ew_data->id}}">
                ...
                <div class="custCol-4">
                    <div class="inputHolder ">
                        ...
                        <select class="chosen-select" name="employee_id" id="employee_id">
                           ...
                           <option value='{{$eachEmployee->id}}'<?php if($ew_data->employee_id == $eachEmployee->id){ ?> selected="selected"<?php } ?>>{{$eachEmployee->username}}: {{$eachEmployee->first_name}}</option>
                           @endforeach 
                        </select>
                    </div>
                </div>
                    ...
                    <div class="inputHolder ">
                        ...
                        <select class="chosen-select" name="warehouse_id" id="warehouse_id">
                           ...
                           @foreach($warehouses as $eachWarehouse)
                            <option value='{{$eachWarehouse->id}}' <?php if($ew_data->warehouse_id == $eachWarehouse->id){ ?> selected="selected"<?php } ?>>{{$eachWarehouse->name}}</option>
                           @endforeach  
                        </select>
                    </div>
                </div>
