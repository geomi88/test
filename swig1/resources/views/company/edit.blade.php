...
<div class="contentHolderV1">
    ...
    <form method="post" action="{{ action('Company\CompanyController@update') }}" id="addcompanyform" enctype="multipart/form-data" class="formTypeV1">
        {{ csrf_field() }}
        <input type="hidden" id="base_path" value="<?php echo url('/'); ?>">
        <input type="hidden" name="company_id" id="company_id" value="{{$company_details->id}}">
        <b>Personal Information</b>
        <div class="formSection">
            <label>First Name</label>
            <input type="text" name="firstName" id="firstName" value="{{$company_details->firstName}}">
            <div class="customClear"></div>
            <div class="firstNameError error"></div>
            <label>Last Name</label>
            <input type="text" name="lastName" id="lastName" value="{{$company_details->lastName}}">
            <div class="customClear"></div>
            <div class="lastNameError error"></div>
            <label>Email Address</label>
            <input type="text" name="email" id="email" value="{{$company_details->email}}">
            <div class="customClear"></div>
            <div class="emailError error"></div>
            <label>Phone Number</label>
            <input type="text" name="phoneNumber" id="phoneNumber" value="+{{$company_details->phoneNumber}}">
            <div class="customClear"></div>
        ...
        <div class="formSection">
            ...
            <input type="text" name="companyName" id="companyName" value="{{$company_details->companyName}}">
            <div class="customClear"></div>
            <div class="companyNameError error"></div>
            <label>Company Description</label>
            <textarea name="companyDescription" id="companyDescription">{{$company_details->companyDescription}}</textarea>
            <div class="customClear"></div>
            <div class="companyDescriptionError error"></div>

            <label>Company Logo</label>
            <img src="<?php echo url('/'); ?>{{$company_details->companyLogo}}" style="width:150px;height:150px;">
