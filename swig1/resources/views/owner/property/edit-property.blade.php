			...
			<div class="agentAdminHolder">
				<div class="text-capitalize adminTitle">
					<h1>Edit property details</h1>
                        ...
                        <form method="POST" action="<?php echo url('/'); ?>/owner/save-property-updates" enctype="multipart/form-data" id="addproperty" >
                            {{ csrf_field() }}
				...
				<div class="mainBoxHolder">
					...
					<div class="row text-capitalize mt-4">
						<div class="col-6">
							...
							<div class="halfWidth mb-3">
								<div class="radioCheckStyleHolder float-left">
									<input id="planning_permits1" name="planning_permits" type="radio" <?php if($property->planning_permits=="1"){ echo "checked";} ?>>
									<label for="planning_permits1"><span>yes</span></label>
								</div>
								<div class="radioCheckStyleHolder float-left">
									<input id="planning_permits2" name="planning_permits" type="radio" <?php if($property->planning_permits=="0"){ echo "checked";} ?>>
							...
							<div class="halfWidth mb-3 pr-3">
								<select class="inputStyle chosen-search" id="neighborhood" name="neighborhood">
									...
									<option value="{{$neighbors->id}}"  <?php if ($property->beighborhood == $neighbors->id) { echo "selected";}?>>{{$neighbors->name}}</option>
                                                                    @endforeach
								</select>
							</div>
							<div class="clearfix"></div>
						...
						<div class="col-9 mb-3">
							...
							<label class="labelStyle">Add gallery images</label>
										...
										<div class="galleryAddHolder">
                                            ...
                                            <input type="hidden" name="galleryCount" id="galleryCount" value="{{count($gallery)}}">
					</div>
...
function previewFile_add(field, previewField, imgHW) {
    ...
    function removeImg(id){
        ...
        $.ajax({
          url: '<?php echo url('/')?>/agent/image-remove',
          data: {"imgId": id},
          type: 'POST',
          async: true,
          dataType: "json",
