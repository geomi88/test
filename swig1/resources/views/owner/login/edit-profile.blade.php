...
@section('content')
		...
		<div class="agentAdminHolder">
			...
			<div class="mainBoxHolder">
				<form method="POST" action="<?php echo url('/'); ?>/owner/update-account" enctype="multipart/form-data" id="saveAgent" >
                                              {{ csrf_field() }}
				  <div class="row">
                                                <div class="col-3 mb-3">
						 <label class="labelStyle text-capitalize">Name <span class="redColor">*</span></label>
						</div>
                                                <input type='hidden' name='profile_id' id='profile_id' value='{{\Crypt::encrypt($agentData->id)}}'>
                                                <div class="col-9 mb-3">
                                                     <input class="inputStyle" name='name' id='name' type="text" placeholder="Enter Name" value='{{$agentData->name}}'>
                                                </div>
						...
						<div class="col-9 mb-3">
							<div class="previewImageHolder">
                                                             <?php
                                                                ...
                                                                if ($agentData->image == "" || $agentData->image == NULL) {
                                                                    ?>

                                                                    <?php
                                                                } else {
                                                                    $image = url('/') . '/uploads/agent/' . $agentData->image;
