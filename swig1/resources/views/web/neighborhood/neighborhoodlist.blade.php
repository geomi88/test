...
@section('content')
@section('title', 'Neighborhood List')
                ...
                <section class="contentWrapper neigh">
                    <div class="pageCenter">
                        <ul class="breadcrumbsHolder">
                            ...
                            <li>
                                <a href="javascript:void();">{{__('neighborhoods')}}</a>
                            </li>
                        </ul>
                        <div class="row headingSection">
                            <div class="small-12 medium-12 large-12 column">
                                <h2>{{__('neighborhoods')}}</h2>
                                <p class="text">Click on the neighborhood for more information or view the properties in that neighborhood</p>
                            </div>
                        </div>
                        <div class="row small-up-1 medium-up-3 large-up-3">
                           
                            @foreach($neighborhood_list as $neighborhoods)
                             <?php
                                    $image = 'default_image/property_default.png';

                                    if ($neighborhoods->images == "" || $neighborhoods->images == NULL) {
                                        ?>

                                        <?php
                                    } else {
                                        $image = url('/') . '/uploads/neighborhood/' . $neighborhoods->images;

                                        if (!@getimagesize($image)) {
                                            $image = 'default_image/property_default.png';
                                        }else{
                                            $image ='/uploads/neighborhood/' . $neighborhoods->images;
                            ...
                            <div class="column constructionList">
                                <figure>
                                    <a href="{{ URL::to('web/neighborhood/neighborhood-view', ['id' => Crypt::encrypt($neighborhoods->id)]) }}">
                                        <img src="{{URL::asset($image)}}" alt="">
                                    <div class="mask">
                                        <div class="textContent center">
                                            <span>{{$neighborhoods->name}}</span>
                                        </div>
                                    </div>
                                    </a>
                                </figure>
