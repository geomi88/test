	...
	customJS = {
		...
		common : {
			commonJS : function(){
				...
				/*$('body').on( 'click','.dropList ',function(e){
					$(this).find('p').next('ul').stop(true, true).slideToggle() ;
					$(this).siblings('.dropList').find('p').next('ul').stop(true, true).slideUp();
				});
				
				$('.dropList ul').slideUp();
				
				$('body').on( 'click','.dropList li a',function(){
					var val =$(this).text();
					$(this).parent('li').parent('ul').parent('.dropList').find('p').text(val);
				});
				
			 	$(document,'.dropList p').on("click", function(event){
			        var $trigger = $(".dropList");
			        if($trigger !== event.target && !$trigger.has(event.target).length){
			            $(".dropList ul").slideUp();
			        }        
			    });
			    $('.advnceFilterHolder ul li').click(function(e){
			    	e.stopPropagation();
			    });*/
