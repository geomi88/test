...
<div class="innerContent">
    ...
    <form method="post" action="{{ url('requisitions/purchase_order_list/download_pfd') }}" id="download_pdf_form">
    <a class="btnAction print right bgGreen" href="#">Print</a>
    <a class="btnAction saveDoc bgBlue" href="#" id="<?= (($orderdata->mailed_status == 1) ? '' : 'btnsendemail')  ?>" style="margin-bottom: 15px; <?= (($orderdata->mailed_status == 1) ? 'opacity: 0.5;' : '')  ?>">Send To Supplier</a>
    <a class="btnAction saveDoc bgOrange" href="#" id="<?= (($orderdata->send_warehouse_status == 1) ? '' : 'btnsendtowarehouse')  ?>" style="margin-left: 15px; <?= (($orderdata->send_warehouse_status == 1) ? 'opacity: 0.5;' : '')  ?>">Send To Warehouse</a>
    <a class="btnAction dwnldDoc bgBlue" href="#" id="download_pdf" style="margin-left: 15px;">Download PO</a>
...
<script>
    $('body').on('click', '#btnsendemail', function () {
        ...
        if($('#btnsendemail').length == 1){
            
        var blnConfirm = confirm("Are you sure to send PO to supplier?");
           if (!blnConfirm) {
               return;
           }
            ...
            $.ajax({
                ...
                success: function (return_data) {
                    if (return_data == 1) {
                        ...
                        $("#btnsendemail").css("opacity","0.5");
                        $("#btnsendemail").removeAttr('id');
                    }
                    if (return_data == -1) {
    ...
    $("#btnsendtowarehouse").click(function(data){
        if($('#btnsendtowarehouse').length == 1){
            
        var blnConfirm = confirm("Are you sure to send PO to warehouse?");
           if (!blnConfirm) {
               return;
           }
            ...
            $.ajax({
                type: 'POST',
                url: '{{ url("requisitions/sendto_warehouse") }}',
                data: {po_id: $('#po_id').val()},
                beforeSend: function () {
                    $('.commonLoaderV1').show();
                },
                success: function (return_data) {
                    if (return_data == 1) {
                        toastr.success('PO Has Been Send To Warehouse');
                        $("#btnsendtowarehouse").css("opacity","0.5");
                        $("#btnsendtowarehouse").removeAttr('id');
                    }
                    if (return_data == -1) {
                        toastr.error('Failed To Send To Warehouse');
