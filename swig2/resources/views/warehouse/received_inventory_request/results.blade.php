...
<tr>
  ...
  <td>{{$item_request->request_id}}</td>
    ...
    <td>{{$item_request->branch_name}}</td>
    <td>{{$item_request->warehouse}}</td>
                         
    <td class="btnHolder">
        <div class="actionBtnSet">
            <a class="btnAction action bgGreen" href="{{ URL::to('warehouse/received_inventory_request/showdetails', ['id' => Crypt::encrypt($item_request->id)]) }}">View</a>
        </div>
    </td>
</tr>
@endforeach
