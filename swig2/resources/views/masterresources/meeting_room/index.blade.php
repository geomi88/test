...
<script>
    ...
    $(document).ready(function ()
    {
     ...
     $('.takePrint').click(function () {
        ...
        if(!$("#slno").is(':checked')&&!$("#name").is(':checked')&&!$("#alias").is(':checked')) {
        ...
        var strStyle='<style>.paginationHolder {display:none;}';
         ...
         if(!$("#slno").is(':checked')){
                strStyle+=' .meeting_slno {display:none;}';
         }
         if(!$("#name").is(':checked')){
                strStyle+=' .meeting_room {display:none;}';
         }
         if(!$("#alias").is(':checked')){
               strStyle+=' .meeting_alias {display:none;}';
         }
        strStyle+='.actionBtnSet{display:none;} .listerType1 tr:nth-of-type(2n) {background: rgba(0, 75, 111, 0.12) none repeat 0 0;}</style>';
         
    win.document.write(strStyle+'<div style="text-align:center;"><h1>Meeting Rooms</h1></div><table style="width: 100%; border: 3px solid #3088da" cellspacing="0" cellpadding="0" class="listerType1">'+
                                    '<thead style="background: rgba(0, 0, 0, 0) linear-gradient(to bottom, rgba(109, 179, 242, 1) 0%, rgba(84, 163, 238, 1) 50%, rgba(54, 144, 240, 1) 51%, rgba(30, 105, 222, 1) 100%) repeat  0 0;">'+
                                        '<tr class="headingHolder">'+
                                            '<td style="padding:10px 0;color:#fff;" class="meeting_slno"> Sl.No.</td>'+
                                            '<td style="padding:10px 0;color:#fff;" class="meeting_room"> Name</td>'+
                                            '<td style="padding:10px 0;color:#fff;" class="meeting_alias"> Alias </td>'+
                                        '</tr>'+
...
<div class="innerContent">
    <form id="pdfgenerator" action="{{ url('masterresources/meeting_room/exportdata') }}" method="post">
    ...
    <div class="fieldGroup" id="fieldSet1">
        <div class="custRow">
            <div class="custCol-12">
                <a href="{{ action('Masterresources\MeetingroomController@add') }}" id="btnNew" class="right commonBtn commonBtn bgGreen addBtn">Create</a>
            </div>
        </div>
    ...
    <div class="printChoose">
                        <div class="custRow">
                            ...
                            <div class="custCol-4">
                                <div class="commonCheckHolder checkRender">
                                    <label>
                                        <input id="name" checked="" type="checkbox">
                                        <span></span>
                                        <em>Name</em>
                                    </label>
                                </div>
            ...
            <div id="tblcategorytable">
                <table cellpadding="0" cellspacing="0" style="width: 100%;">
                    ...
                    <tbody class="categorybody" id='categorybody'>
                        @include('masterresources/meeting_room/result')
...
<script>
    ...
    $(function () {
        ...
        $('#reset').click(function () {
            ...
            $('#page-limit').val(10);
            //search();
            window.location.href = '{{url("masterresources/meeting_room")}}';
        });
    });
