...
<script>
    ...
    $(document).ready(function ()
    {
     ...
     $('.takePrint').click(function () {
        ...
        if(!$("#slno").is(':checked')&&!$("#bankName").is(':checked')&&!$("#alias").is(':checked')) {
        ...
        var strStyle='<style>.paginationHolder {display:none;}';
         ...
         if(!$("#slno").is(':checked')){
                strStyle+=' .bank_slno {display:none;}';
         }
         if(!$("#bankName").is(':checked')){
                strStyle+=' .bank_name {display:none;}';
         }
         if(!$("#alias").is(':checked')){
               strStyle+=' .bank_alias {display:none;}';
         }
         strStyle+='.actionBtnSet{display:none;} .listerType1 tr:nth-of-type(2n) {background: rgba(0, 75, 111, 0.12) none repeat 0 0;}</style>';
         
    win.document.write(strStyle+'<div style="text-align:center;"><h1>Banks List</h1></div><table style="width: 100%; border: 3px solid #3088da" cellspacing="0" cellpadding="0" class="listerType1">'+
                                    '<thead style="background: rgba(0, 0, 0, 0) linear-gradient(to bottom, rgba(109, 179, 242, 1) 0%, rgba(84, 163, 238, 1) 50%, rgba(54, 144, 240, 1) 51%, rgba(30, 105, 222, 1) 100%) repeat  0 0;">'+
                                        '<tr class="headingHolder">'+
                                            '<td style="padding:10px 0;color:#fff;" class="bank_slno"> Sl.No.</td>'+
                                            '<td style="padding:10px 0;color:#fff;" class="bank_name"> Bank Name</td>'+
                                            '<td style="padding:10px 0;color:#fff;" class="bank_alias"> Alias </td>'+
                                        '</tr>'+
                                    '</thead>'+ $('.bankbody')[0].outerHTML +'</table>');
        win.document.close();
        win.print();
        win.close();
        return false;
...
<div class="innerContent">
    <form id="pdfgenerator" action="{{ url('masterresources/bank/exportdata') }}" method="post">
    <header class="pageTitle">
        <h1>Banks <span>List</span></h1>
    </header>	
    <div class="fieldGroup" id="fieldSet1">
        <div class="custRow">
            <div class="custCol-12">
                <a href="{{ action('Masterresources\BankController@add') }}" id="btnNew" class="right commonBtn commonBtn bgGreen addBtn">Create</a>
            </div>
        </div>
        ...
        <div class="listerType1 reportLister"> 
            ...
            <div id="tblregion">
                <table cellpadding="0" cellspacing="0" style="width: 100%;">
                    ...
                    <thead class="listHeaderBottom">
                        ...
                        <tr class="headingHolder">
                            ...
                            <td class="filterFields">
                                <div class="custRow">
                                    <div class="custCol-12">
                                        <input type="text" id="searchbyname" name="searchbyname" placeholder="Enter Bank Name">
                    ...
                    <tbody class="bankbody" id='bankbody'>
                        @include('masterresources/bank/bank_result')
...
<script>
    ...
    function search()
    {
        ...
        $.ajax({
            ...
            success: function (return_data) {
                if (return_data != '')
                {
                    $('.bankbody').html(return_data);
                    $(".commonLoaderV1").hide();
                }
                else
                {
                    $(".commonLoaderV1").hide();
                    $('.bankbody').html('<p class="noData">No Records Found</p>');
                }
            }
        });
    }
    ...
    $(function () {
        ...
        $('#reset').click(function () {
            ...
            $('#page-limit').val(10);
            //search();
            window.location.href = '{{url("masterresources/bank")}}';
        });
    });
