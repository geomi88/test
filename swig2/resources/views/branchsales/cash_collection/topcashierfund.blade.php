  ...
  $(document).ready(function()
{
     $(document).on('click', '.pagination a',function(event)
    {
       ...
       getData(page);
        ...
        if(!$("#date").is(':checked')&&!$("#employeeCode").is(':checked')&&!$("#supervisorNamee").is(':checked')&&!$("#amountt").is(':checked')) {
        ...
        var strStyle='<style>.paginationHolder {display:none;}';
         ...
         if(!$("#date").is(':checked')){
                strStyle+=' .top_date {display:none;}';
         }
         if(!$("#employeeCode").is(':checked')){
                strStyle+=' .top_code {display:none;}';
         }
         if(!$("#supervisorNamee").is(':checked')){
               strStyle+=' .top_namee {display:none;}';
         }
         if(!$("#amountt").is(':checked')){
              strStyle+=' .top_amount {display:none;}';  
         } 
        strStyle+='.actionBtnSet{display:none;} .listerType1 tr:nth-of-type(2n) {background: rgba(0, 75, 111, 0.12) none repeat 0 0;}</style>';
         
    win.document.write(strStyle+'<div style="text-align:center;"><h1>Top Cashier Cash Collections</h1></div><table style="width: 100%; border: 3px solid #3088da" cellspacing="0" cellpadding="0" class="listerType1">'+
				'<thead style="background: rgba(0, 0, 0, 0) linear-gradient(to bottom, rgba(109, 179, 242, 1) 0%, rgba(84, 163, 238, 1) 50%, rgba(54, 144, 240, 1) 51%, rgba(30, 105, 222, 1) 100%) repeat  0 0;">'+
					'<tr class="headingHolder">'+
						'<td style="padding:10px 0;color:#fff;" class="top_date"> Date</td>'+
						'<td style="padding:10px 0;color:#fff;" class="top_code"> Employee Code</td>'+
						'<td style="padding:10px 0;color:#fff;" class="top_namee"> Supervisor Name </td>'+
						'<td style="padding:10px 0;color:#fff;" class="top_amount"> Amount </td>'+
...
<div class="contentArea">
    ...
    <div class="innerContent">
        <form id="transfer_cash_collection" action="{{ url('branchsales/cash_collection/exporttopdf') }}" method="post" enctype="multipart/form-data">
    ...
    <div class="printChoose">
        <div class="custRow">
            ...
            <div class="custCol-4">
                <div class="commonCheckHolder checkRender">
                    <label>
                        <input id="supervisorNamee" checked="" type="checkbox">
                        <span></span>
                        ...
                        <div class="custRow">
                            ...
                            <div class="custCol-4">
                                <div class="inputHolder">
                                    ...
                                    <input type="file" onchange="readURL(this);" id="attachment" name="attachment" accept="image/*,.doc,.docx,.txt,.pdf,application/msword" /> 
                                </div>
                            </div>
                            <div class="custCol-4">
                                <div class="inputHolder">
...
$(function () {        
    ...
    $('#btnSaveTransfer').click(function() {
        ...
        if(blnConfirm){
            var documents = new FormData($('#transfer_cash_collection')[0]);
            documents.append('strCollectionIds', strCollectionIds);
       ...
       function search()
    {
        ...
        $.ajax({
            type: 'POST',
            url: 'topcashierfund',
            data: { searchkey: searchkey, aorder: aorder,empSearch:empSearch, aamount: aamount, startdate: startdate, enddate: enddate},
            beforeSend: function () {
                $(".commonLoaderV1").show();
            },
            success: function (return_data) {
