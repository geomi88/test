...
use App\Models\Training_attendees;
...
class TrainingController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $meetings = DB::table('tasks')
                ->select('tasks.*')
                ->join('training_attendees', 'training_attendees.training_id', '=', 'tasks.id')
                ->whereRaw("tasks.task_type = 5 and (training_attendees.user_id = $login_id or tasks.owner_id = $login_id)")
        ...
        if ($request->ajax()) {
            ...
            return view('training/training_result', array('meetings' => $meetings));
        }

        return view('training/training_list', array('meetings' => $meetings));
    }

    public function createtraining(Request $request, $id = '') {
        $paginate = Config::get('app.PAGINATE');
        ...
        if ($request->ajax()) {
            ...
            return view('training/emp_result', array('employees' => $employees, 'arrid' => $arrId));
        }
        return view('training/createtraining', array('employees' => $employees, "meetingdate" => "", 'job_positions' => $job_positions, 'arrid' => $arrId, 'meeting_rooms' => $meeting_rooms));
    }
    ...
    public function savemeeting() {
        try {
            ...
            foreach ($attendees as $attendee) {
                $attendees_model = new Training_attendees();
                $attendees_model->training_id = $model->id;
...
//                    $message->to($attendee_email)->subject('Training Schedule');
//                });
                ...
                $category = 'Training';
                $message = 'Training ' . Input::get('title') . ' has been scheduled';
                // Notification to next level
                Auth::user()->notify(new TrainingNotification($from, $to, $message, $category, $parent_meeting_id, $type));
                //Notification ends
            }
...
//                    $message->to($guest->guest_email)->subject('Training Schedule');
//                });
            }
                        ...
                        for ($i = strtotime($daily_selected_day, strtotime($start_date)); $i <= $endDate; $i = strtotime('+1 week', $i)) {
                            ...
                            if ($start_date != $meeting_start_time) {
                                ...
                                foreach ($attendees as $attendee) {
                                    ...
                                    $message = 'Training ' . Input::get('title') . ' has been scheduled';
                                    // Notification to next level
                                    Auth::user()->notify(new TrainingNotification($from, $to, $message, $category, $model->id, $type));
                                    //Notification ends
                                }
                                foreach ($guests as $guest) {
                                    $attendees_model = new Training_attendees();
...
//                        });
            ...
            Toastr::success('Training Added Successfully!', $title = null, $options = []);
            return Redirect::to('training/training_list');
        } catch (\Exception $e) {
    ...
    public function view($training_id) {
        try {
            ...
            $meeting_details = DB::table('tasks')
                            ...
                            ->whereRaw("tasks.id = $training_id")->first();
            
            $training_attendees = DB::table('training_attendees')
                            ->select('training_attendees.*', 'employee_details.first_name', 'employee_details.last_name', 'employee_details.username', 'employee_details.email')
                            ->join('employees as employee_details', 'employee_details.id', '=', 'training_attendees.user_id')
                            ->whereRaw("training_attendees.training_id = $training_id")->get();
            
            $training_guests = DB::table('training_attendees')
                            ->select('training_attendees.*')
                            ->whereRaw("training_attendees.training_id=$training_id AND user_id IS NULL")->get();

            return view('training/view', array('meeting_details' => $meeting_details, 'meeting_attendees' => $training_attendees,'training_guests'=>$training_guests));
        } catch (\Exception $e) {
            Toastr::error('Sorry There Was Some Problem!', $title = null, $options = []);
            return Redirect::to('training/training_list');
        }
...
//                        }
        ...
        if (count($arrDates) > 0) {
            ...
            $meetings = DB::table('tasks')
                    ->select('tasks.*')
                    ->whereRaw("($strWhere)")
                    ->whereRaw("date(start_date)>='$curDate' AND tasks.task_type IN (3,5) AND meeting_room=$room_id")
                    ->get();
        }
...
//                        }
        ...
        if (count($arrDates) > 0) {
            ...
            $meetings = DB::table('tasks')
                    ...
                    ->whereRaw("($strWhere)")
                    ->whereRaw("date(start_date)>='$curDate' AND tasks.task_type IN(3,5) AND (owner_id=$empid OR user_id=$empid)")
                    ->distinct("training_attendees.training_id")
                    ->get();
        }
    ...
    public function edit($training_id) {
        try {
            ...
            return view('training/edit', array('meeting_details' => $meeting_details));
        } catch (\Exception $e) {
            Toastr::error('Sorry There Was Some Problem!', $title = null, $options = []);
            return Redirect::to('training/training_list');
        }
    ...
    public function updateparticipation() {
        try {
            ...
            $update_availability = DB::table('training_attendees')
                    ->where(['user_id' => $user_id, 'training_id' => $training_id])
                    ->update(['availability_status' => 2, 'comment' => $reason]);
            $training_id = \Crypt::encrypt($training_id);
            Toastr::success('Updated Successfully!', $title = null, $options = []);
            return Redirect::to("training/view/$training_id");
        } catch (\Exception $e) {
            Toastr::error('Sorry There Was Some Problem!', $title = null, $options = []);
            return Redirect::to('training/training_list');
        }
...
//            DB::table('tasks')->where('id', '=', $taskid)->delete();
            ...
            Toastr::success('Training Deleted Successfully', $title = null, $options = []);
            return Redirect::to('training/training_list');
        } catch (\Exception $e) {
            Toastr::error('Sorry There Was Some Problem!', $title = null, $options = []);
            return Redirect::to('training/training_list');
