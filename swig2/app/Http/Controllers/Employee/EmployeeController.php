...
class EmployeeController extends Controller {
    ...
    public function index(Request $request, $id = '') {
        ...
        if ($request->ajax()) {
            ...
            $search_pass_no = Input::get('search_pass_no');
            $search_gossi_no = Input::get('search_gossi_no');
            ...
            $employees = DB::table('employees')
                    ...
                    ->when($search_key, function ($query) use ($search_key) {
                     ...
                     ->when($search_pass_no, function ($query) use ($search_pass_no) {
                        return $query->whereRaw("(employees.passport_number like '$search_pass_no%')");
                    })
                    ->when($search_gossi_no, function ($query) use ($search_gossi_no) {
                        return $query->whereRaw("(employees.gossi_number like '$search_gossi_no%')");
                    })
                    ...
                    ->when($profession, function ($query) use ($profession) {
                        return $query->where('employees.id_professional', '=', $profession);
                    })
        ...
        return view('employee/index', array('employees' => $employees, 'job_positions' => $job_positions, 'countries' => $countries, "job_id" => $job_id, "divisions" => $divisions, 'country_id' => $country_id,'id_professional'=>$id_professional));
    }
    ...
    public function add() {
        ...
        $employees = DB::table('employees')
         ...
         $regions = DB::table('master_resources')
                        ->select('master_resources.*')
                        ->where(['resource_type' => 'REGION', 'status' => 1])
                        ->orderby('name', 'ASC')->get();
        ...
        return view('employee/add', array('title' => 'Add Employee', 'description' => '', 'page' => '', 'religions' => $religions,
            'gender_list' => $gender_list, 'job_positions' => $job_positions, 'companies' => $companies, 'divisions' => $divisions, 'modules' => $modules, 'countries' => $countries, 'employees' => $employees,'id_professional'=>$id_professional,'regions'=>$regions));
    }
       ...
       unset($employee_data['top_manager']);
        ...
        if (Session::get('employee_id')) {
            ...
            DB::table('ac_accounts')
                        ->whereRaw("type_id=$employee_id AND type='Employee'")
    ...
    public function Disable($id) {
        try {
            ...
            DB::table('ac_accounts')
                    ->whereRaw("type_id=$dn AND type='Employee'")
                    ->update(['status' => 0]);
    ...
    public function edit($id) {
        ...
        $id_card_pic = DB::table('documents')
                ...
                ->where('document_type', '=', 'ID_CARD')
                 ->where('document_owner_type', '=', 'EMPLOYEE')
                ->first();
        if ($id_card_pic == '') {
            $id_pic = '';
        } else {
        ...
        $start_pic = DB::table('documents')
                ...
                ->where('user_id', '=', $id)
                ->where('document_owner_id', '=', $id)
                ->where('document_type', '=', 'START_FROM')
                 ->where('document_owner_type', '=', 'EMPLOYEE')
                ->first();
        if ($start_pic == '') {
            $st_pic = '';
        ...
        $per_pic = DB::table('documents')
                ...
                ->where('user_id', '=', $id)
                ->where('document_owner_id', '=', $id)
                ->where('document_type', '=', 'PERSONAL_PROFILE')
                 ->where('document_owner_type', '=', 'EMPLOYEE')
                ->first();
        if ($per_pic == '') {
            $pp_pic = '';
        ...
        $qu_pic = DB::table('documents')
               ...
               ->where('user_id', '=', $id)
                ->where('document_owner_id', '=', $id)
                ->where('document_type', '=', 'QUALIFICATION')
                ->where('document_owner_type', '=', 'EMPLOYEE')
                ->first();
        if ($qu_pic == '') {
            $q_pic = '';
    ...
    public function fetch_notification() {
        ...
        foreach ($notification as $notifications) {
            ...
            if($details->category=='Purchase Requisition')
            {
                $url='purchase_requisition';
            }
            
            if($details->category=='Import Purchase Requisition')
            {
                $url='import_purchase_requisition';
            }
            ...
            if($details->category=='Maintainance Requisition')
            {
                $url='maintainance_requisition';
            }
            
            if($details->category=='Service Requisition')
            {
                $url='service_requisition';
            }
            if($details->category=='Leave Requisition')
            {
                $url='leave_requisition';
            }
             if($details->category=='Advance Payment Requisition')
            {
                $url='advance_payment_requisition';
            }
             if($details->category=='Payment Advice')
            {
                $url='payment_approval';
            }
            if($details->category=='Payment Advice.')
            {
                $url='payment_advice';
            }
            if($details->category=='Owner Drawings Requisition')
            {
                $url='drawing_requsition';
            }
          ...
          if ($details->category == 'Inventory Request') {
            ...
            else if ($details->category == 'Make Payment') {
                echo '<a  class="not_id ' . $old . ' " id=' . $notifications->id . ' value="' . $notifications->id . ' "href="' . $public_path . '/requisitions/payment_advice/add/' . \Crypt::encrypt($details->notifiable_id) . '">
                                    <span>' . $details->category . '</span>
                                    <em>' . date("d-m-Y", strtotime($notifications->created_at)) . '</em>
                                    <b>' . $details->message . '</b>
                                </a>';
                ...
                echo '<a  class="not_id ' . $old . ' " id=' . $notifications->id . ' value="' . $notifications->id . ' "href="' . $public_path . '/requisitions/' . $url . '/' . $details->type . '/' . \Crypt::encrypt($details->notifiable_id) . '">
                                    <span>' . $details->category . '</span>
                                    <em>' . date("d-m-Y", strtotime($notifications->created_at)) . '</em>
                                    <b>' . $details->message . '</b>
                                </a>';
    ...
    public function upload_documents() {
        try {
            ...
            if (Input::hasFile('indentpic')) {
                ...
                $documentmodel->document_owner_id = $employee_id;
                $documentmodel->document_owner_type = 'EMPLOYEE';
        ...
        try {
            ...
            if (Input::hasFile('indentpic')) {
                ...
                $id = DB::table('documents')
                        ...
                        ->where('document_type', 'ID_CARD')
                        ->where('document_owner_id', $employee_id)
                        ->where('document_owner_type', '=', 'EMPLOYEE')
                        ->first();
                if (count($id) > 0) {
...
//        });
    ...
    public function editlist(Request $request, $id = '') {
        ...
        $employees = DB::table('employees')
                ->select('employees.*', 'job_position.id as job_position_id', 'job_position.name as job_position_name', 'division.id as division_id', 'division.name as division_name', 'country.name as country_name','profession.name as professionname')
                ->leftjoin('master_resources as job_position', 'job_position.id', '=', 'employees.job_position')
                ->leftjoin('master_resources as division', 'division.id', '=', 'employees.division')
                ->leftjoin('master_resources as profession', 'employees.id_professional', '=', 'profession.id')
                ->leftjoin('country', 'country.id', '=', 'employees.nationality')
                ->where('employees.status', '!=', 2)
                ->where('employees.admin_status', '=', 0)
                ->when($job_id, function ($query) use ($job_id) {
        ...
        $professions = DB::table('master_resources')
                    ->select('master_resources.*')
                    ->where(['resource_type' => 'ID_PROFESSION', 'status' => 1])
                    ->orderby('name', 'ASC')
                    ->get();
        ...
        return view('employee/editlist', array('employees' => $employees, 'job_positions' => $job_positions, 'countries' => $countries,"job_id"=>$job_id,"divisions"=>$divisions,'country_id'=>$country_id,'professions'=>$professions));
    }
        ...
        if($excelorpdf=="Excel"){
            ...
            Excel::create('EmployeeList', function($excel) use($employees){
                ...
                $excel->sheet('Employee List', function($sheet) use($employees){
                    ...
                    $sheet->setCellValue('D3', 'Employee List');
                    ...
                    $sheet->row(5, array('Sl No', 'Employee Code','Employee Name','Gossi Number','Profession',"Country","Passport Number","Email Id","Phone No","Division","Status"));
...
//                        $cells->setBorder('thin', 'thin', 'thin', 'thin');
                    ...
                    for($i=0;$i<count($employees);$i++){
                        ...
                        $sheet->setCellValue('E'.$chrRow, str_replace('_',' ',$employees[$i]->id_professional_name));
                        $sheet->setCellValue('F'.$chrRow, $employees[$i]->country_name);
                        $sheet->setCellValue('G'.$chrRow, $employees[$i]->passport_number);
                        ...
                        $sheet->setCellValue('K'.$chrRow, $status);
                            
                        $sheet->cells('A'.$chrRow.':K'.$chrRow, function($cells) {
                            $cells->setFontSize(9);
...
//                            $cells->setBorder('thin', 'thin', 'thin', 'thin');
            ...
            <body style="margin: 0; padding: 0;font-family:DejaVu Sans;">
                ...
                <div style="text-align:center;"><h2>Employee List</h2></div>
                    <table style="width: 100%; border: 3px solid #3088da" cellspacing="0" cellpadding="0" class="listerType1">
                        <thead style="background: rgba(0, 0, 0, 0) linear-gradient(to bottom, rgba(109, 179, 242, 1) 0%, rgba(84, 163, 238, 1) 50%, rgba(54, 144, 240, 1) 51%, rgba(30, 105, 222, 1) 100%) repeat  0 0;">
                            <tr class="headingHolder">
                                ...
                                <td style="padding:10px 5px;color:#fff;"> Profession</td>
                                <td style="padding:10px 5px;color:#fff;"> Country </td>
                                <td style="padding:10px 5px;color:#fff;"> Passport Number </td>
            ...
            foreach ($employees as $employee) {
                ...
                $html_table .='<tr>
                                    ...
                                    <td style="color: #535352; font-size: 12px;padding: 10px 5px;">' . $employee->passport_number . '</td>
            ...
            return $pdf->download('mtg_employee_list.pdf');
        }
    }   
