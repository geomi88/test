...
class SupervisorBranchesSales extends AbstractWidget {
    ...
    public function run() {
        ...
        if ($employee_data->privilege_status == 1) {
            ...
            $total_sales = DB::table('pos_sales')
                    ...
                    ->groupby(DB::raw("pos_sales.branch_id"))
                    ->orderby('total', 'DESC')
                    ->whereRaw("date(pos_sales.pos_date) > '$search_date' and pos_sales.added_by_user_type = 'Supervisor' and pos_sales.status=1 and  pos_sales.branch_id in (select branch_id from resource_allocation where employee_id = $employee_id and active = 1)")
                    ->get();
        }
        ...
        return view("widgets.supervisor_branches_sales", [
